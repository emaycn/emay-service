...
public class AppUtils {
	...
	public synchronized static String genAppCode(int length) {
		...
		if (length == 3) {
			digitRandom += SERVICECODE_BEGIN_3[index];
		} else {
			digitRandom += SERVICECODE_BEGIN_6[index];
		}
		for (int i = 0; i < length - 1; i++) {
			digitRandom += rand.nextInt(10);
		}
		return digitRandom;
	}
	...
	public static Result checkAppCode(String code ) {
		...
		if(code.length() == 3) {
			...
			boolean ok = Arrays.stream(SERVICECODE_BEGIN_3).filter(num -> num.equals(first)).count() > 0;
			return ok ? Result.rightResult() : Result.badResult("3位服务号只能是 6,8,9,1,0开头");
		}else if(code.length() == 6) {
			String first = code.substring(0, 1);
			boolean ok = Arrays.stream(SERVICECODE_BEGIN_6).filter(num -> num.equals(first)).count() > 0;
