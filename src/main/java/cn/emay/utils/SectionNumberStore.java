...
public class SectionNumberStore {
	private Logger logger = LoggerFactory.getLogger(SectionNumberStore.class);
	private EmayTreeNode<Character, SectionNumber> root = new EmayTreeNode<Character, SectionNumber>('1', null);

	public SectionNumberStore(SectionNumber... numbers) {
		if(numbers == null || numbers.length == 0) {
			return ;
		}
		Arrays.stream(numbers).forEach(number -> save(number));
	}
	...
	public void save(SectionNumber number) {
		...
		for (char mobilenode : mobilenodes) {
			...
			if (node == null) {
				node = new EmayTreeNode<Character, SectionNumber>(mobilenode, null);
